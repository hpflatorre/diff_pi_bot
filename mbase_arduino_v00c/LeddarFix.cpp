...
char LeddarOne::parseResponse()
{
  ...
  while (millis()-startTime < 10) 
  {
    if (SerialPort.available())
    {
      //if (len >= 25) return ERR_LEDDAR_BAD_RESPONSE;  //QUICK FIX to accept the extra null byte after message
      //if (len >= 25) break;
      dataBuffer[len++] = SerialPort.read();
      if (len == 1 && dataBuffer[0] == NULL) len = 0;   //QUICK FIX to accept the extra null byte before message
      startTime = millis();
    }
    //if (len == 25 && !SerialPort.available())         //QUICK FIX to accept the extra null byte after message
    if (len == 25 )
    {
      break;
    }
...
// **********************************************************************
char LeddarOne::setBaud(uint8_t baud_code)
{
  unsigned char dataBuffer[19] = {0};
  unsigned int i = 0;
  unsigned long startTime = millis();
  ...
  // Write configuration to permanent memory
  dataBuffer[19] = {0};
  i = 0;
  startTime = millis();
