#ifndef LEDDAR_ONE_H
#define LEDDAR_ONE_H

//Error codes
#define ERR_LEDDAR_BAD_CRC -1
#define ERR_LEDDAR_NO_RESPONSE -2
#define ERR_LEDDAR_BAD_RESPONSE -3
#define ERR_LEDDAR_SHORT_RESPONSE -4
#define ERR_LEDDAR_SERIAL_PORT -5
// Number of detections does not match 
#define ERR_LEDDAR_NB_DETECTIONS -6;

#define LEDDAR_ONE_BUFFER_SZ	50
#define MODBUS_MAX_LEN			256 //  Header 2, max payload 152, CRC 2

#define ARRAYSIZE(v) (sizeof(v)/sizeof(v[0]))        // helper macro to get the number of elements of an array

#include "ros/ros.h"
#include <sstream>
#include <cstdio>
#include <serial/serial.h>

using std::string;

/// Represents a measurement:
struct detection
{
  unsigned int distance;
  unsigned int amplitude;
  
  // Default constructor
  detection() : distance(0xFFFF), amplitude(0) { }
};

class leddar_one {
  public:
  	detection detections[3]; // LeddarOne sensor can return a maximum of 3 detections
  	
	leddar_one(string port_path, int baud_rate);
	int get_reading();
	char request_reading();

  private:
  	int baud;
  	unsigned char NbDet;
  	unsigned long TimeStamp;
  	std::vector< uint8_t > buffer;
  	serial::Serial leddar_s;

	leddar_one();
	bool CRC16(uint8_t *aBuffer, uint8_t aLength, bool aCheck);
	bool CRC16(std::vector< uint8_t > &aBuffer, uint8_t aLength, bool aCheck);
	
};

// Table of CRC values for high�order byte
static uint8_t CRC_HI[] =
{
    
};

// Table of CRC values for low�order byte
static uint8_t CRC_LO[] =
{
    
};

#endif
